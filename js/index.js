...
function generateFilds(num) {
  ...
  for (var i = 0; i < num; i++) {
    code += '<tr><td><input type="number" name="pi1fl_'+i+'"></td>'+
    '<td><input type="number" name="pi1f2_'+i+'"></td></tr>';
  }
  code += '</table><button type="button" id="bcalculate1">Расчитать</button><button type="button" id="breset">Сбросить</button>';
  $('#filds').append(code);
  $('#filds').fadeIn(400);
}
...
var main = function() {
  ...
  $(document).on("click", "#bcalculate1", function(event) {
    ...
    // Проверка полей распределения возможностей на пустоту
    ...
    if (error == 0) {
      ...
      var arrpk = [];
      $('#results').empty().html();
      code = '<br><table class="table table-hover table-striped text-center"><caption>Результаты анализа</caption><tr><th>№</th><th>&pi;1</th><th>&pi;2</th><th>r1</th><th>r2</th><th>w1</th><th>w2</th><th>&pi;k</th></tr>';
      ...
      for (var i = 0; i < elemnum; i++) {
        ...
        arrpk[i] = tmp[4];
        code += '<tr><td>'+(i+1)+'</td><td>'+arrpi1[i]+'</td>'+'<td>'+arrpi2[i]+'</td>'+'<td>'+tmp[0].toFixed(2)+'</td>'+'<td>'+tmp[1].toFixed(2)+
        '</td>'+'<td>'+tmp[2]+'</td><td>'+tmp[3]+'</td><td>'+tmp[4]+'</td></tr>';
      }
